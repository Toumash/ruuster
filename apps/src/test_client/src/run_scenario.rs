...
impl ProcessManager {
    ...
    fn prepare_producer_command(
        ...
        let result = match metadata {
            ...
            Some(metadata_arg) => {
                format!(
                    "{} --server-addr {} --destination {} --messages-produced {} --message-payload-bytes {} --delay-ms {} --metadata {}",
                    producer_path.display(),
                    self.args.server_addr.clone(),
                    destination,
                    messages_produced,
                    message_payload_bytes,
                    delay_ms,
                    serde_json::to_string(metadata_arg).unwrap()
                )
            }
        };
    ...
    fn create_producer_children(&self) -> Result<Vec<Child>, Box<dyn std::error::Error>> {
        ...
        for producer in &self.config.producers {
            ...
            debug!(value=%producer_command, "producer command");
